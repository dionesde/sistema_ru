...
class agendamento {
    ...
    public function checkIndsp($data) {
        $sql = "SELECT * FROM `dias_invalidos` WHERE data='" . date('Y-m-d', $data) . "'";
    ...
    public function getTable($inicio, $fim, $almoco, $janta, $distribuicao) {
        $str = '';
        ...
        for ($data = $inicio; $data <= $fim; $data+=(1 * 24 * 60 * 60)) {
            ...
            if ($distribuicao == 'checked') {

                if ($_SESSION['beneficio'] == '0') {
                    $str.="<tr><td>" . date('j/m/Y', $data) . "</td><td>Distribuição</td><td><span class='error'>Você não possue benefício.</sapn></td></tr>";
                } else {
                    $saida = $this->checar($data, 3);
                    $str.="<tr><td>" . date('j/m/Y', $data) . "</td><td>Distribuição</td><td>" . $saida['msg'] . "</td></tr>";
                }
            }
        }
        $str.="</tbody>";
    ...
    public function salvar($inicio, $fim, $almoco, $janta, $distribuicao) {
